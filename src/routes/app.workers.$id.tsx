...
export const Route = createFileRoute("/app/workers/$id")({
  head: () => ({ meta: [{ title: "Worker · YOYO ERP" }] }),
...
function WorkerDetailPage() {
  ...
  const stats = useMemo(() => {
    ...
    return { count: logs.length, hours: (totalMin / 60).toFixed(1), open };
  }, [logs]);
    ...
    <div className="space-y-4 pb-24">
      <div className="flex items-center justify-between gap-2">
        <Button asChild variant="ghost" size="sm" className="gap-1.5">
          <Link to="/app/workers"><ArrowLeft className="h-4 w-4" /> Workers</Link>
        </Button>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-semibold tracking-tight truncate">{worker?.name ?? "Worker"}</h1>
              {worker && (
      ...
      <section className="rounded-xl border border-slate-200 bg-white">
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description="Tap Log work to record a shift for this worker."
