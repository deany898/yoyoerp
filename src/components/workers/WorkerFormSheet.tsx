...
export function WorkerFormSheet({ open, onOpenChange, worker, showPayroll, onSaved }: Props) {
  ...
  async function save() {
    ...
    const updatePayload: Database["public"]["Tables"]["workers"]["Update"] = {
      name: name.trim(),
      phone: m,
      is_active: isActive,
      job_role: "worker",
    ...
    const insertPayload: Database["public"]["Tables"]["workers"]["Insert"] = {
      code: "", // auto-generated by trg_auto_code_workers
      name: name.trim(),
      phone: m,
      is_active: isActive,
      job_role: "worker",
    ...
    const res = isEdit
      ? await supabase.from("workers").update(updatePayload).eq("id", worker!.id).select("id").maybeSingle()
      : await supabase.from("workers").insert(insertPayload).select("id").maybeSingle();
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
        ...
        <div className="mt-6 space-y-4">
          ...
          <AutoCodeField label="Employee code" value={worker?.code ?? ""} />
        </div>
