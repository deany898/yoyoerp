...
-- Helper: convert a qty in `from_code` to the equivalent qty in `to_code`
-- when both share the same base_uom. Returns NULL when codes are missing
-- or incompatible — caller should treat NULL as "no conversion, use as-is".
CREATE OR REPLACE FUNCTION public.uom_convert_qty(
  _qty numeric,
  _from_code text,
  _to_code text
) RETURNS numeric
...
BEGIN
  ...
  IF _from_code IS NULL OR _to_code IS NULL THEN RETURN _qty; END IF;
  IF lower(_from_code) = lower(_to_code) THEN RETURN _qty; END IF;

  SELECT factor, base_uom INTO f_factor, f_base
  FROM public.uoms WHERE lower(code) = lower(_from_code) LIMIT 1;
  SELECT factor, base_uom INTO t_factor, t_base
  FROM public.uoms WHERE lower(code) = lower(_to_code) LIMIT 1;

  IF f_factor IS NULL OR t_factor IS NULL THEN RETURN _qty; END IF;
  IF f_base <> t_base THEN RETURN _qty; END IF;
  IF t_factor = 0 THEN RETURN _qty; END IF;

  -- qty in base = _qty * f_factor; convert back to to_code:
  RETURN (_qty * f_factor) / t_factor;
END;
$$;

-- Replace recalc_variant_cost with v2 (landed cost + UOM-normalized BOM + max scrap rule)
...
BEGIN
  ...
  -- UOM factor for this variant (price normalization to base unit)
  ...
  ELSE
    ...
    IF v_manual IS NOT NULL AND v_manual > 0 THEN
      ...
      v_has_quote := true;
    ELSE
      -- Latest active+approved quote, with full landed cost components
      ...
      IF v_quote_unit IS NOT NULL THEN
        ...
        -- landed price per quoted UOM = unit_price + (sum of charges spread over MOQ)
        v_quote_landed := COALESCE(v_quote_unit, 0)
          + (COALESCE(v_quote_freight,0) + COALESCE(v_quote_transport,0)
            ...
            / v_quote_moq;
        -- normalize to base UOM (₹78.50/Kg ÷ 1000 = ₹0.0785/gm)
        v_purchase := v_quote_landed / v_uom_factor;
        v_has_quote := true;
      ELSE
        -- Fall back to last 5 receipts (already landed_cost)
    ...
    IF v_bom_id IS NOT NULL THEN
      ...
      -- UOM-normalized BOM material cost.
      -- bl.qty_per is in bl.uom; component effective_cost is per its own product UOM (base unit).
      -- Convert qty_per → component's product UOM, then multiply by effective_cost.
      SELECT COALESCE(SUM(
    ...
    -- Stage cost (non-packing). Compute BOTH:
    --   (a) compounded: SUM(cost * (1 + rej/100))
    --   (b) yield-based: SUM(cost) / (1 - max_rej/100)
    -- Take MAX per the conservative rule.
    SELECT
      COALESCE(SUM((labour_cost+machine_cost+utility_cost+mould_cost+overhead_cost+qc_cost)*(1+rejection_pct/100.0)),0),
      COALESCE(SUM(labour_cost+machine_cost+utility_cost+mould_cost+overhead_cost+qc_cost),0),
...
-- Manual recompute helper (admin/manager only) used by the UI button.
CREATE OR REPLACE FUNCTION public.recompute_variant_cost(_variant_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  PERFORM public.recalc_variant_cost(_variant_id, 0);
  SELECT effective_cost, purchase_cost, manufacture_cost, cost_updated_at
    INTO v_row FROM public.product_variants WHERE id = _variant_id;
  RETURN jsonb_build_object(
    'variant_id', _variant_id,
    'effective_cost', v_row.effective_cost,
    'purchase_cost', v_row.purchase_cost,
    'manufacture_cost', v_row.manufacture_cost,
    'cost_updated_at', v_row.cost_updated_at
  );
END;
$$;

REVOKE ALL ON FUNCTION public.recompute_variant_cost(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.recompute_variant_cost(uuid) TO authenticated;
